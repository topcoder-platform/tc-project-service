...
module.exports = [
  ...
  (req, res, next) => {
    ...
    let fields = req.query.fields
      ? decodeURIComponent(req.query.fields).split(',')
      : PHASE_ATTRIBUTES;
    let sort = req.query.sort
      ? decodeURIComponent(req.query.sort)
      : 'startDate';
    const memberOnly = req.query.memberOnly
      ? req.query.memberOnly.toLowerCase() === 'true'
      : false;
      ...
      .then((doc) => {
        ...
        if (memberOnly && !isAdmin) {
          phases = _.filter(phases, (phase) =>
            _.includes(
              _.map(_.get(phase, 'members'), 'userId'),
              req.authUser.userId,
            ),
          );
        }
        ...
        return util
          .populatePhasesWithMemberDetails(phases, req)
          ...
          return models.Project.findByPk(projectId, {
            ...
            if (!project) {
              const apiErr = new Error(
                `active project not found for project id ${projectId}`,
              );
              apiErr.status = 404;
              return next(apiErr);
            }
            ...
            _.remove(PHASE_ATTRIBUTES, (attribute) =>
              _.includes(['deletedAt', 'deletedBy'], attribute),
            );
            fields = _.intersection(fields, [
              ...PHASE_ATTRIBUTES,
              'products',
