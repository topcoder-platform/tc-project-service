  ...
  describe('POST /projects/{projectId}/phases/{phaseId}/approvals', () => {
    ...
    it.skip('should return 400 when startDate field is missing', (done) => {
      request(server)
        ...
        .end((err, res) => {
          ...
          validateError(
            resJson,
            'validation error: "startDate" is required,' +
    ...
    it.skip('should return 400 when expectedEndDate field is missing', (done) => {
      request(server)
        ...
        .end((err, res) => {
          ...
          validateError(
            resJson,
            'validation error: "expectedEndDate" is required',
          );
          done();
        });
    });
    ...
    it('should return 400 when decision field is invalid', (done) => {
      request(server)
        ...
        .end((err, res) => {
          ...
          validateError(
            resJson,
            'validation error: "decision" must be one of [approve, reject]',
          );
          done();
        });
    });
    ...
    it('should return 400 when comment field is invalid', (done) => {
      request(server)
        ...
        .end((err, res) => {
          ...
          validateError(
            resJson,
            'validation error: "comment" is not allowed to be empty',
          );
          done();
        });
    });
    ...
    it('should return 400 when endDate is before startDate', (done) => {
      request(server)
        ...
        .end((err, res) => {
          const resJson = res.body;
          resJson.message.should.be
            .a('string')
            .and.satisfy((message) =>
              message.startsWith(
                'validation error: "endDate" must be larger than or equal to',
              ),
            );
          done();
        });
    ...
    it('should return 400 when phase status is not in_review', (done) => {
      request(server)
        ...
        .end((err, res) => {
          ...
          validateError(
            resJson,
            `Phase with id ${phaseId} must be in_review status to make approval`,
          );
          done();
        });
    });
