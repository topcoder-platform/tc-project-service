 ...
 */
const timelineAddedHandler = Promise.coroutine(function* (
  logger,
  msg,
  channel,
) {
  ...
  try {
    ...
    logger.debug(
      `timeline indexed successfully (timelineId: ${data.id})`,
      result,
    );
    channel.ack(msg);
  } catch (error) {
 ...
 * @param  {Object} channel channel to ack, nack
 */
const timelineUpdatedHandler = Promise.coroutine(function* (
  logger,
  msg,
  channel,
) {
  ...
  try {
    ...
    logger.debug(
      `timeline updated successfully in elasticsearh index, (timelineId: ${data.original.id})`,
    );
    channel.ack(msg);
  } catch (error) {
    logger.error(
      `failed to get timeline document, (timelineId: ${data.original.id})`,
      error,
    );
    channel.nack(msg, false, !msg.fields.redelivered);
  }
 ...
 * @param  {Object} channel channel to ack, nack
 */
const timelineRemovedHandler = Promise.coroutine(function* (
  logger,
  msg,
  channel,
) {
  ...
  const data = JSON.parse(msg.content.toString());
  try {
    yield eClient.delete({ index: ES_TIMELINE_INDEX, id: data.id });
    logger.debug(
      `timeline deleted successfully from elasticsearh index (timelineId: ${data.id})`,
    );
    channel.ack(msg);
  } catch (error) {
    logger.error(
      `failed to delete timeline document (timelineId: ${data.id})`,
      error,
    );
    channel.nack(msg, false, !msg.fields.redelivered);
  }
...
const findProjectPhaseProduct = function (logger, productId) {
  // eslint-disable-line func-names
  let product;
...
async function timelineAdjustedKafkaHandler(app, topic, payload) {
  ...
  if (timeline && timeline.reference === TIMELINE_REFERENCES.PRODUCT) {
    ...
    createEvent(
      BUS_API_EVENT.PROJECT_PLAN_UPDATED,
      {
