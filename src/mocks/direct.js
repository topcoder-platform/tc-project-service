...
router.route('/v3/direct/projects')
    .get((req, res)=>{
        ...
        res.json(util.wrapResponse(req.id, {projects: projects}))
    })
    .post((req, res)=>{
...
router.route('/v3/direct/projects/:projectId(\\d+)/billingaccount')
    .post((req, res)=>{
        ...
        app.logger.info({body: req.body, projectId: projectId }, "add billingaccount to Project")
...
router.route('/v3/direct/projects/:projectId(\\d+)/copilot')
    .post((req, res)=>{
        const projectId = req.params.projectId
        app.logger.info({body: req.body, projectId: projectId }, "add copilot to Project")
        if(projects[projectId]) {
            projects[projectId] = _.merge(projects[projectId], req.body)
    ...
    .delete((req, res)=>{
        const projectId = req.params.projectId
        app.logger.info({body: req.body, projectId: projectId }, "remove copilot from Project")
        if(projects[projectId]) {
